function App(){
    ...
    function multiply (num) {
        return 10*num.toFixed(2); // 소숫점 자리까지 반올림하여 문자열로 반환하는 함수
    }
    
    console.log('자바 스크립트 배열 관련 map 함수');
    const numbers = [4,9,16,21];

    // .map = 자바의 확장for문과 같은 기능/주어진 배열만큼 반복하여 출력.
    ...
    console.log('화살표 함수와 String객치의 lengh()함수를 같이 사용해봅니다.');
...
if(sentence.length <= mylength) {
    ...
    const WinterItems = () => {
        const items = [
            ...
            {name:'얼음', description:'차갑고 단단쓰~'},
            {name:'눈', description:'하얗게 내림'},
    ...
    return(
        ...
        </div> // li는 result05안에 있으니 바깥을 ul로 감싸기
    );
};
