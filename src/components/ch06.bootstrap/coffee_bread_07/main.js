    ...
    */
    ...
    const Ordering = (orderInformation) => {
        ...
        products.sort((a, b) => {
            ...
            // 정렬힐 컬럼이 '카테고리'이면, 한글로 변환해서 비교를 수행함.
            if (column === 'category') {
                const aCategory = categories.find(c => c.EngName === a.category);
                const bCategory = categories.find(c => c.EngName === b.category);

                aValue = aCategory ? aCategory.KorName : a.category;//한글이름
                bValue = bCategory ? bCategory.KorName : b.category;//한글이름
            }
    ...
    /* 사용자가 '정렬할 컬럼' 또는 '정렬 방식'을 선택하면 이 함수가 실행됨. */
    const ClickOrderBy = (column_name, order_by) => {
        const newOrderInfo = {
            column: column_name,
            ordering: order_by
        }; // 변경된 정렬 정보
        setOrderInfo(newOrderInfo);// 변경내용을 State에 반영시킴.

        Ordering(newOrderInfo);
    }

    /* 필드 검색 기능 추가 */
    /* 필드 검색 시 내가 선책한 category의 영문 이름이 저장되는 state. */
    const [filterCategory, setFilterCategory] = useState(null);

    /* 사용자가 카테고리 콤보박스ㅔ= */
    const CategoryChanged = (selelctedCategory) => {
        setFilterCategory(selelctedCategory); // 선택된 카테고리의 이름이 변경됨.

        // 필터링이 이루어지면, 정렬 함수를 다시 실행시켜 화면을 갱신.
        Ordering(orderInfo);
    }

    // 필드 검색을 사용해 필터링을 수행할지 말지 결정하는 불리언 타입의 변수
    const isFilterinNeeded = filterCategory && filterCategory !== 'all';

    //삼항 연산자를 사용하여 선택된 카테고리와 동일한 품목들만 필터링
    const filterProducts = isFilterinNeeded
    ...
    return (
        ...
        <Card>
            ...
            <Card.Body>
                ...
                <Content Contents={filterProducts}
